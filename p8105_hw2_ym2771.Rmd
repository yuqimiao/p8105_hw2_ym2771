...
* The total precipitation in 2018 is `r sum(precip_2017 %>% pull(Total))`
...
* By merging snp and unemployment data into pols_month, we have the final dataset, FiveThirtyEight, with `r nrow(unemployment)` rows * `r ncol(unemployment)` columns, for the range of the year in different set is different, some values are missing.
...
The most frequent name among boys is `r name_freq[which.max(name_freq$frequency),1]`

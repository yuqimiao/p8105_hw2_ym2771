...
This dataset contains `r nrow(precip_data)` observations and `r ncol(precip_data)` variables, giving information on monthly total precipitation (`r names(precip_data)[3]`) in 2017-2018.
...
* The second data frame "snp" has the dimension of `r nrow(snp)` rows * `r ncol(snp)` columns, the range of year is from `r range(snp %>% pull(year))[1]` to `r range(snp %>% pull(year))[2]`, and the names of the key variables are `r names(snp)[3]`

* The third data frame "unemployment" has `r nrow(unemployment)` rows * `r ncol(unemployment)` columns, the range of year is from `r range(unemployment %>% pull(year))[1]` to `r range(unemployment %>% pull(year))[2]`, and the names of the key variables are `r names(unemployment)[3]`

* By merging snp and unemployment data into pols_month, we have the final dataset, FiveThirtyEight, with `r nrow(FiveThirtyEight)` rows * `r ncol(FiveThirtyEight)` columns, the range of year is from `r range(FiveThirtyEight %>% pull(year))[1]` to `r range(FiveThirtyEight %>% pull(year))[2]`,for the range of the year in different set is different, some values are missing.
...
g <- ggplot(data = plot_data, aes(x= rank, y = count, label = childs_first_name)) +
  ...
  geom_text_repel(
    data = subset(plot_data, rank <= 10),
    nudge_x = 50,
    direction = "y",
    segment.size = 0.2
  ) +
  xlab("rank of the names")+ ylab("number of children with a name") 
g
```
